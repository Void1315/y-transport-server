...
type User struct {
	...
	IdNumber   string `gorm:"size:30;DEFAULT:''"`          // 列名为 `id_number` 身份证号码
	Phone      string `gorm:"size:20;DEFAULT:'';NOT NULL"` // 列名为 `phone` 手机号码
	Password   string `gorm:"size:100;DEFAULT:''"`         // 列名为 `password` 密码的sha256值
	Salt       string `gorm:"size:20;DEFAULT:''"`          // 列名为 `salt` 密码盐
	Token      string `gorm:"size:512;DEFAULT:''"`         // 列名为 `token` jwt登录凭证
	Type       bool   `gorm:"NOT NULL;DEFAULT:0"`          // 列名为 `type` 用户类型 0本站 1第三方
}
